...
module.exports = {
    ...
    register: async function (req,res) {
        ...
        if (userRecord) {
            ...
            let affiliateUserNameResult;
            let userCreationResult;
            if (params.registrationType === 'affiliate') {
                affiliateUserNameResult = await sails.helpers.generateAffiliateCode(`${params.name}${new Date().getTime()}`);
            ...
            } else if (params.registrationType === 'student') {
                // Student registration logic
                // studentUserNameResult = await sails.helpers.generateStudentCode(`${params.name}${new Date().getTime()}`);
                userCreationResult = await Users.create({
                    name: params.name,
                    roleId: 2, // Assuming roleId 2 corresponds to student role
                    email: params.email.toLowerCase(),
                    password: md5(params.password) ,
